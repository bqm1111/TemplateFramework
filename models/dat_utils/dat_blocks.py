...
class LocalAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # padding workaround
        pad_l = pad_t = 0
        pad_r = (self.window_size[1] - W %
                 self.window_size[1]) % self.window_size[0]
        pad_b = (self.window_size[0] - H %
        ...
        q, k, v = [einops.rearrange(
            t, 'b n (h c1) -> b h n c1', h=self.heads) for t in [q, k, v]]
        ...
        if mask is not None:
            ...
            attn = einops.rearrange(attn, '(b n) h w1 w2 -> b n h w1 w2', n=nW,
                                    h=self.heads, w1=ww, w2=ww) + mask.reshape(1, nW, 1, ww, ww)
        ...
        x = einops.rearrange(x, '(b r1 r2) (h1 w1) c -> b c (r1 h1) (r2 w1)', r1=r1,
                             r2=r2, h1=self.window_size[0], w1=self.window_size[1])  # B x C x H x W
...
class ShiftWindowAttention(LocalAttention):
    ...
    def forward(self, x):
        ...
        mask_windows = einops.rearrange(
            img_mask, '(r1 h1) (r2 w1) -> (r1 r2) (h1 w1)', h1=self.window_size[0], w1=self.window_size[1])
        attn_mask = mask_windows.unsqueeze(
            1) - mask_windows.unsqueeze(2)  # nW ww ww
        attn_mask.masked_fill_(attn_mask != 0, float(-100.0)
                               ).masked_fill(attn_mask == 0, float(0.0))

        shifted_x = torch.roll(
            x, shifts=(-self.shift_size, -self.shift_size), dims=(2, 3))
        sw_x = super().forward(shifted_x, attn_mask)
        x = torch.roll(sw_x, shifts=(self.shift_size,
...
class DAttentionDCN(nn.Module):
    ...
    def __init__(
        ...
        self.proj_off = nn.Linear(
            self.nc, self.n_heads * self.sample_per_pt * 2)
    ...
    def _init_projections(self):
        ...
        grid_init = (grid_init / grid_init.abs().max(-1, keepdim=True)
                     [0]).view(self.n_heads, 1, 2).repeat(1, self.sample_per_pt, 1)
    ...
    def forward(self, x):
        ...
        attn = self.proj_att(einops.rearrange(
            x, 'b c h w -> b (h w) c'))  # B (H W) (h Nk)
        attn = einops.rearrange(attn, 'b l (h n) -> b l h n',
                                h=self.n_heads, n=self.sample_per_pt)  # B (H W) h Nk
        attn = self.attn_drop(F.softmax(attn, dim=3))

        offsets = self.proj_off(einops.rearrange(
            x, 'b c h w -> b (h w) c'))  # B (H W) (h Nk 2)
        off = einops.rearrange(offsets, 'b l (h n p) -> b l h n p',
                               h=self.n_heads, n=self.sample_per_pt, p=2)  # B (H W) h Nk 2
        ref = self.reference.reshape(
            1, H * W, 1, 1, 2).expand(B, -1, self.n_heads, self.sample_per_pt, -1)  # B (H W) h Nk 2
        pos = ref + off

        v = self.proj_v(x)
        v_ = einops.rearrange(v, 'b (a d) h w -> (b a) d h w',
                              a=self.n_heads, d=self.n_head_channels)  # (B h) hc H W

        pos = einops.rearrange(pos, 'b l a n p -> (b a) l n p',
                               a=self.n_heads, n=self.sample_per_pt)  # (B h) (H W) Nk 2

        v_ = F.grid_sample(input=v_, grid=pos[..., (1, 0)].to(
            v_.dtype), mode='bilinear', align_corners=True)  # (B h) hc (H W) Nk

        attn_ = einops.rearrange(
            attn, 'b l a n -> (b a) l n')[:, None, :, :]  # (B h) 1 (H W) Nk

        v = attn_.mul(v_).sum(dim=3)  # (B h) hc (H W)
        v = einops.rearrange(v, '(b a) d (h w) -> b (a d) h w',
                             a=self.n_heads, d=self.n_head_channels, h=H, w=W)  # B C H W
        ...
        if self.use_pe and (not self.no_off):
            if self.dwc_pe:
                ...
                self.rpe_table = nn.Parameter(
                    torch.zeros(self.n_heads, self.q_h *
                                self.q_w, self.kv_h * self.kv_w)
                )
    ...
    def forward(self, x):
        ...
        if self.use_pe and (not self.no_off):
            ...
            if self.dwc_pe:
                ...
                attn_bias = F.interpolate(attn_bias, size=(
                    H * W, n_sample), mode='bilinear', align_corners=True)
                attn = attn + \
                    attn_bias.reshape(B * self.n_heads, H * W, n_sample)
            elif self.log_cpb:
                ...
                displacement = torch.sign(
                    displacement) * torch.log2(torch.abs(displacement) + 1.0) / np.log2(8.0)
                # B * g, H * W, n_sample, h_g
                ...
                displacement = (q_grid.reshape(B * self.n_groups, H * W, 2).unsqueeze(
                    2) - pos.reshape(B * self.n_groups, n_sample, 2).unsqueeze(1)).mul(0.5)
