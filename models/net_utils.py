...
class MPG(nn.Module):
    ...
    def forward(self, x, x_d):
        if self.need_depth_embedding:
            x_d = self.depth_embedding(x_d)
        x, x_d = self.FRM(x, x_d)
        return x, x_d
...
class MFA(nn.Module):
    ...
    def __init__(self, num_token, token_dim, reduction_ratio, num_heads, num_head_channels, qkv_bias=True) -> None:
        ...
        self.num_heads = num_heads
        reduction_dim = in_query_dim // reduction_ratio
        head_dim = reduction_dim // num_heads
        self.scale = head_dim ** -0.5
        self.num_token = num_token
        self.num_head_channels = num_head_channels
        self.channel_per_head_reduce = num_head_channels // reduction_ratio
        assert self.channel_per_head_reduce > 0

        self.adapter_token = nn.Embedding(num_token, token_dim)
        self.norm = nn.LayerNorm(token_dim)
        self.linear_kv = nn.Linear(
            token_dim, 2 * reduction_dim)

        self.linear_q = nn.Linear(
            in_query_dim, reduction_dim)
        # self.qkv = nn.Linear(reduction_dim, 3 * reduction_dim, bias=qkv_bias)
        self.q_proj = nn.Linear(reduction_dim, reduction_dim, bias=qkv_bias)
        self.k_proj = nn.Linear(reduction_dim, reduction_dim, bias=qkv_bias)
        self.v_proj = nn.Linear(reduction_dim, reduction_dim, bias=qkv_bias)
    ...
    def forward(self, query):
        ...
        query should have shape of B * N * C
...
if __name__ == '__main__':
    ...
    in_query_dim = num_heads * num_head_channels
    reduction_ratio = 32
    mfa = MFA(num_token, token_dim, reduction_ratio,
              num_heads, num_head_channels)
    example_query = torch.ones(B, 19200, in_query_dim)
    mfa(example_query)
