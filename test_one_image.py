...
def setup_model(cfg_file, device):
    ...
    # )
    checkpoint_path = "pretrained/mit_b2.pth"
...
if __name__ == "__main__":
    ...
    cfg_file = "config/semseg/nyuv2/mit_b2.yaml"
    rgb = Image.open("data/NYUDepthv2/image/0.jpg").convert(
        "RGB"
    )
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    rgb = np.array(rgb)
    depth = np.load("data/NYUDepthv2/depth/0.npy")
    data = preprocess(rgb, depth)
